...
contextBridge.exposeInMainWorld('coreAPI', {
	...
	downloadMedia: (format: videoFormat, title: string, url: string) => ipcRenderer.send('media:download', format, title, url),
	downloadAndMergeMedia: (formats: { details: videoFormat, type: 'audio' | 'video' }[], durationSec: number, title: string, url: string) => ipcRenderer.send('media:downloadAndMerge', formats, durationSec, title, url),
