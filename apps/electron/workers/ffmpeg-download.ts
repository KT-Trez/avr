...
const downloadRecording = async (
  ...
  ffmpeg()
      .input(
          await YTDownload(recordingURL, {
            filter: (vFormat: videoFormat) =>
                vFormat.audioBitrate === recordingFormat.audioBitrate &&
                vFormat.qualityLabel === recordingFormat.qualityLabel &&
                vFormat.codecs === recordingFormat.codecs,
          }),
      )
      ...
      .on('progress', (progress: FFmpegProgress) => {
        if (progress.percent) mediaDownloadPercent = progress.percent;
        else
          mediaDownloadPercent =
              (convertTimestampToSeconds(progress.timemark) / recodingMetadata.recordingDurationSec) * 100;
