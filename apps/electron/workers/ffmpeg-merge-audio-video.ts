...
import type { FFmpegProgress } from '../../typings/interfaces-core';
...
const mereAudioWithVideo = (recordingPaths: string[], savePath: string) => {
  ...
  for (const recordingPath of recordingPaths) command.input(recordingPath);
  ...
  command
      ...
      .on('end', () => {
        ...
        for (const recordingPath of recordingPaths) fs.unlinkSync(recordingPath);
