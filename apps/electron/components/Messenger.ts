...
class Messenger {
  ...
  public static notify(message: string, severity: NotificationSeverity, title?: string, variant?: NotificationVariant) {
    if (!win.webContents) throw Error('Window not initialized');
  ...
  public static notifyQueueProgress() {
    ...
    if (new Date().getTime() <= this.lastProgresUpdate + 1500) return;
...
function mountIpcMainHandlers() {
  fs.readdir(path.resolve(__dirname, '../ipc-main'), async (err, filesNames) => {
    if (err) throw Error('Cannot read directory with ipcMain handlers: ' + err.message);
