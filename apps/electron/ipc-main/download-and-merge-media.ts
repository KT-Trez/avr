...
const handler: IpcMainHandler = {
  execute: async (
      ...
      for (const format of formats) {
        ...
        const worker = new Worker(path.join(getPath('workers'), 'ffmpeg-download.js'), { workerData })
            .on('exit', exitCode => {
              ...
              else Cache.updateDownloadStatus(downloadID, 'downloading ' + format.type === 'audio' ? 'video' : 'audio');
            })
            .on('error', err => {
              ...
              switch (message.type) {
                ...
                case 'download-progress':
                  ...
                  Cache.updateDownloadProgress(
                      downloadID,
                      false,
                      Math.round((audioPercent + mergePercent + videoPercent) / 3),
                  );
    ...
    const worker = new Worker(path.join(getPath('workers'), 'ffmpeg-merge-audio-video.js'), { workerData })
        .on('exit', exitCode => {
          if (exitCode !== 0) throw Error('Worker ended with code: ' + exitCode);
