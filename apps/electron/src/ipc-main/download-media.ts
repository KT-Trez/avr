...
//     let fileExt = 'mp4';
//     if (format.hasAudio && !format.hasVideo) fileExt = 'mp3';
//
//     // set filename, id and path to save selected media
//     const mediaID = YTDownload.getVideoID(url);
//     const filename = [format.audioBitrate ?? '0', format.qualityLabel ?? '0p', mediaID, crypto.randomUUID()]
//         .join('_')
//         .concat('.', fileExt);
...
//           vFormat.audioBitrate === format.audioBitrate && vFormat.qualityLabel === format.qualityLabel,
//     })
...
//                   Messenger.notify('Media [' + mediaID + '] saved at [' + filename + ']',
// NotificationSeverity.Success); }), ); }, name: 'media:download', type: 'on', };  export default handler;
