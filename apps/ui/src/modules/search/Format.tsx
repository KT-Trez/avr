...
function Format({
  ...
  const selectAudio = () => {
    if (isSelected) setAdvancedAudioFormat(null);
    else setAdvancedAudioFormat(recordingFormat);
    setIsSelected(value => !value);
  };

  const selectVideo = () => {
    if (isSelected) setAdvancedVideoFormat(null);
    else setAdvancedVideoFormat(recordingFormat);
    setIsSelected(value => !value);
  };
  ...
  return (
      <TableRow>
        ...
        <Fade in={advancedDownload} unmountOnExit={true}>
          <TableCell align={'center'}>
            <Box sx={{ height: 30.75 }}>
              <Switch
                  disabled={hasAdvancedAudioFormat && !isSelected}
                  onChange={selectAudio}
                  size={'small'}
                  sx={{ visibility: recordingFormat.hasAudio && !recordingFormat.hasVideo ? 'initial' : 'hidden' }}
              />
              <Switch
                  disabled={hasAdvancedVideoFormat && !isSelected}
