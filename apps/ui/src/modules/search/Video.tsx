...
import type { Video as IVideo } from '../../../typings/interfaces';
...
function Video({ video }: VideoProps) {
  ...
  const advanceDownload = () => {
    if (!advancedAudioFormat || !advancedVideoFormat) return;
  ...
  const showDescription = () => {
    if (isDownloadMenuVisible) setIsDownloadMenuVisible(false);
  ...
  const showDownloadMenu = async () => {
    ...
    if (isDescriptionVisible) setIsDescriptionVisible(false);
  ...
  return (
      <Paper>
        <Grid container={true} item={true}>
          ...
          <Grid item={true} sx={{ position: 'relative' }} xs={true}>
            <Stack sx={{ p: 1 }}>
              <Typography noWrap={false} variant={'h6'}>
                {video.title}
              </Typography>
              <Typography variant={'body1'}>{video.author.name}</Typography>
            </Stack>
          ...
          <Grid item={true} sx={{ p: 1, position: 'relative' }} xs={2}>
            ...
            <Typography sx={{ bottom: 0, mb: 1, mr: 1, position: 'absolute', right: 0 }}>{video.ago ??
                'N/A'}</Typography>
          </Grid>
        ...
        <Collapse in={isDownloadMenuVisible}>
          <Grid item={true} sx={{ p: 1 }} xs={12}>
            ...
            <Collapse in={isAdvancedDownloadActive} unmountOnExit={true}>
              <Stack>
                <Stack alignItems={'flex-end'} direction={'row'}>
                  <Typography variant={'body2'}>Audio format:</Typography>
                  <Typography sx={{ ml: 1 }} variant={'caption'}>
                    {advancedAudioFormat
                        ? `${advancedAudioFormat.audioCodec} ${advancedAudioFormat.audioQuality}`
                        : '-'}
                  </Typography>
                </Stack>
                <Stack alignItems={'flex-end'} direction={'row'}>
                  ...
                  <Typography sx={{ ml: 1 }} variant={'caption'}>
                    {advancedVideoFormat
                        ? `${advancedVideoFormat.videoCodec} ${advancedVideoFormat.qualityLabel}`
                        : '-'}
                  </Typography>
                </Stack>
                {/* todo: prettier search button */}
                <Button
                    disabled={!advancedAudioFormat || !advancedVideoFormat}
                  ...
                  <Table size={'small'} stickyHeader={true}>
                    <TableHead>
                      <TableRow>
                        ...
                        <Fade in={isAdvancedDownloadActive} unmountOnExit={true}>
                          <TableCell align={'center'}>Audio/Video</TableCell>
                        </Fade>
                        <TableCell/>
                      </TableRow>
                    </TableHead>
