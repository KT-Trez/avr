...
function Downloads() {
  ...
  useEffect(() => {
    if (pathOfFileToRemove) setIsDialogOpen(true);
  }, [pathOfFileToRemove]);

  return (
      <NoContentView
          emptyText={'No downloaded audio/music.'}
          header={
            <TitleBar>
              <Typography variant={'h6'}>Open in directory</Typography>
      ...
      >
        <RemoveFileDialog
            cancel={cancelRemoveFile} isOpen={isDialogOpen} path={pathOfFileToRemove} remove={removeFile}
        />
        ...
        <TableContainer>
          <Table size={'small'} stickyHeader={true}>
            ...
            <TableBody>
              {files.map(f => (
                  <File key={f.name} metadata={f} setPathOfFileToRemove={setPathOfFileToRemove}/>
              ))}
            </TableBody>
          </Table>
        </TableContainer>
