...
import type { ProgressAction, ProgressType } from '../../typings/enums';
...
export default class Queue {
  ...
  public static addProgress(
      actions: { action: ProgressAction; type: ProgressType; value?: number }[],
      id: string,
      name: string,
  ) {
    for (const action of actions) action.value = 0;
  ...
  public static updateProgress(id: string, progresses: YT_DL.Core.Stats.Progress[]) {
    ...
    if (!entry) return console.error('update referenced missing progress');

    for (const progress of progresses) {
      const p = entry.progresses.find(p => p.action === progress.action && p.type === progress.type);
      if (p) p.value = progress.value;
    }
